...
function normalizeProviderDisplayPayload(providerCode: Provider, payload: ProviderResult) {
  ...
  const specialCells: SlotCell[] = specialSlotLabels.map((slot, index) => {
    let number = '----';
    if (specialFromLayout.size > 0) {
      number = specialFromLayout.get(slot) ?? '----';
    } else {
      // Fallback only: upstream did not provide slot layout, cannot guarantee extracted top3 slot accuracy.
      number = normalizeNumber(payload.special_cells?.[index] ?? payload.special_numbers?.[index]);
    }
    return { slotLabel: slot, number };
  });
  ...
  const finalSpecial = specialCells.map((cell) => ({
    ...cell,
    number: top3SlotSet.has(cell.slotLabel) ? '----' : cell.number,
  }));

  const consolationCells: SlotCell[] = consolationSlotLabels.map((slot, index) => {
    if (consolationFromLayout.size > 0) return { slotLabel: slot, number: consolationFromLayout.get(slot) ?? '----' };
    return { slotLabel: slot, number: normalizeNumber(payload.consolation_numbers?.[index]) };
  });
