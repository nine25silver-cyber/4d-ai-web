-- 4D AI Web/App minimal user_entitlements setup
...
--
-- Scope:
-- - Creates public.user_entitlements only.
...
comment on table public.user_entitlements is
  'Read-only membership/subscription state for frontend clients. Writes must be performed by trusted backend/admin code only.';

comment on column public.user_entitlements.user_id is
...
comment on column public.user_entitlements.is_pro is
