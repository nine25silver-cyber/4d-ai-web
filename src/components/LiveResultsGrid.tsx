...
export function LiveResultsGrid({regionSlug, providers, initialResults}: Props) {
  ...
  return (
    <section>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm">
        <div>
          <span className="font-black text-blue-900">{t('liveLabel')}</span>
          <span className="ml-2 text-slate-600">{t('liveText')}</span>
        </div>
        <div className="text-xs font-semibold text-slate-500">
          {isRefreshing ? t('liveChecking') : lastChecked ? t('liveChecked', {time: new Date(lastChecked).toLocaleTimeString()}) : t('liveWaiting')}
        </div>
      </div>
