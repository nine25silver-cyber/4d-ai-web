...
export async function generateMetadata({params}: {params: Promise<{locale: Locale}>}): Promise<Metadata> {
  ...
  const title = locale === 'zh' ? '今日4D成绩' : locale === 'ms' ? 'Keputusan 4D Hari Ini' : 'Today 4D Results';
  const description = locale === 'zh'
    ? '查看 West Malaysia、East Malaysia、Cambodia、Singapore 今日4D成绩。'
    : locale === 'ms'
      ? 'Lihat keputusan 4D hari ini untuk West Malaysia, East Malaysia, Cambodia dan Singapore.'
      : 'View today 4D results for West Malaysia, East Malaysia, Cambodia and Singapore.';
  return buildMetadata({locale, path: '/results/west-malaysia', title, description});
}
