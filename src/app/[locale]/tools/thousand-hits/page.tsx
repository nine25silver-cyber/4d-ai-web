...
export async function generateMetadata({params}: {params: Promise<{locale: Locale}>}): Promise<Metadata> {
  ...
  const title = locale === 'zh' ? '搜索千字中奖记录 - 4D AI' : locale === 'ms' ? 'Rekod Menang 3D - 4D AI' : '3D Winning Records - 4D AI';
  ...
  return (
    <main className="container-shell py-10">
      ...
      <section className="mt-4 border-b border-slate-200 pb-8">
        <h1 className="text-3xl font-black text-slate-950 sm:text-4xl">{locale === 'zh' ? '搜索千字中奖记录' : locale === 'ms' ? 'Rekod Menang 3D' : '3D Winning Records'}</h1>
      </section>
