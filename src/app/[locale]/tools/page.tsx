...
const toolCards = ['numberSearch', 'thousandHits', 'hotCold', 'packageRanking', 'favorites'] as const;
type ToolCard = (typeof toolCards)[number];
type ExtraToolCard = 'packageRanking' | 'thousandHits';

function hasExtraCopy(tool: ToolCard): tool is ExtraToolCard {
  return tool === 'packageRanking' || tool === 'thousandHits';
}
    ...
    <main>
      ...
      <section className="bg-slate-50 py-14 sm:py-16">
        ...
        {toolCards.map((tool) => {
          ...
          const extraCopy = hasExtraCopy(tool) ? extra[tool] : null;
          ...
          return (
            <article key={tool} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <Link href={href} className="mt-5 inline-flex rounded-md border border-blue-300 bg-blue-50 px-4 py-2 text-sm font-black text-blue-900 hover:bg-blue-100">
                {extraCopy ? extraCopy.cta : t(`${tool}Cta`)}
              </Link>
            </article>
          );
        })}
